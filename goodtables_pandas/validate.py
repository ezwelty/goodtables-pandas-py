...
def validate(source, return_tables=False):
    ...
    for resource, name in zip(resources, names):
        ...
        for foreignKey in foreignKeys:
            ...
            foreignKey['reference']['fields'] = _as_list(foreignKey['reference']['fields'])
    ...
    for resource, name in zip(resources, names):
        ...
        for foreignKey in foreignKeys:
            ...
            keys.append(key)
            resources[i]['schema']['uniqueKeys'] = [list(t)
                for t in set(tuple(l) for l in keys)]
    for resource, name in zip(resources, names):
        required, unique = [], []
        schema = resource['schema']
    ...
    for i, resource in enumerate(package.get('resources', [])):
        ...
        errors += (
            check_constraints(
                dfs[name], schema=resource.get('schema', {})) +
            check_primary_key(
                dfs[name], resource.get('schema', {}).get('primaryKey', []),
                skip_required=True, skip_single=True) +
            check_unique_keys(
                dfs[name], resource.get('schema', {}).get('uniqueKeys', []),
                skip_single=True))
        report['tables'][i]['errors'] += errors
    ...
    for i, resource in enumerate(package.get('resources', [])):
        ...
        errors = check_foreign_keys(
            dfs[name], resource.get('schema', {}).get('foreignKeys', []),
            references=dfs, constraint=None)
        report['tables'][i]['errors'] += errors
    # Update report
    counts = []
    for i, table in enumerate(report['tables']):
        ...
        table['valid'] = count == 0
        table['errors'] = [{k: v for k, v in dict(e).items() if v is not None}
            if isinstance(e, goodtables.Error) else e for e in table['errors']]
        counts.append(count)
        table.pop('time')
